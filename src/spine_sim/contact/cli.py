"""Command line validation entry points for M2."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="spine-m2")
    subcommands = parser.add_subparsers(dest="command", required=True)
    analytic = subcommands.add_parser("validate-analytic")
    analytic.add_argument(
        "--output",
        type=Path,
        default=Path("results/m2_validation/analytic_validation.json"),
    )
    ...
    smoke.add_argument(
        "--output",
        type=Path,
        default=Path("results/m2_validation/m1_ten_terrain_smoke.json"),
    )
