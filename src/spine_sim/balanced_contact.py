...
MODEL_VERSION='ijms-balanced-condensed-contact-5'
...
class CondensedArray:
    ...
    def solve(self,x,preload,previous,guess=None,tolerance=None,max_nfev=80,recover=False):
        ...
        def evaluate(v):
            ...
            residual=np.r_[contact.ravel(),force[:,1].sum()/preload,(force[:,2].sum()-preload)/preload]
        ...
        solved=least_squares(residual,guess,jac=jacobian,x_scale='jac',max_nfev=max_nfev,
        ...
        state.update(residual=float(np.max(np.abs(error))),nfev=solved.nfev,
                     ...
                     requested_preload_N=float(preload))
    ...
    def run(self,start_xy,preload,distance=.01,step=25e-6,progress=None,cpu_budget_s=45,preload_min_divisor=128):
        ...
        def axial_range(trial,phase):
            ...
            stroke=np.flatnonzero([(r.parameters.mount_type=='spring' and s[i]>=r.parameters.max_compression_m)
                                  for i,r in enumerate(self.rods)])
            if not (len(negative) or len(shortened) or len(stroke)):return None
            indices=negative if len(negative) else shortened if len(shortened) else stroke
            reason='AXIAL_EXTENSION_UNSUPPORTED' if len(negative) else 'EXPOSED_LENGTH_LIMIT' if len(shortened) else 'COMPRESSION_STOP_NOT_IMPLEMENTED'
