...
def run_with_placements(array,start_xy,preload,distance,step,*,offsets_m,
                        attempt_cpu_budget_s,total_cpu_budget_s,preload_min_divisor=128,progress=None,state_observer=None):
    ...
    for index,offset in enumerate(offsets_m):
        ...
        observe=(lambda event,state,row:state_observer(event,state,row,index)) if state_observer else None
        ...
        last=array.run(start,preload,distance,step,progress=report,
                       cpu_budget_s=min(attempt_cpu_budget_s,remaining),preload_min_divisor=preload_min_divisor,state_observer=observe)
