...
后续编码先读 [IJMS物理约束](IJMS物理约束.md)，再按具体任务阅读实现和运行文档。

新针形、固定安装和梯度长度补偿已实现。小阵列按 [启动说明](IJMS小阵列启动.md) 与 [固定配置](../experiments/ijms_small_array.json) 执行；[扫描规划](IJMS大规模扫描规划.md) 说明已锁定的材料粗筛和后续机制/大阵列任务边界。

本目录只说明当前程序：系统怎样组成、各模块怎样实现、怎样调用、输入输出是什么，以及模型能回答到哪里。

IJMS 新版请先阅读 [新版机理与接口](IJMS新版机理与接口.md)、[批量仿真交接](IJMS批量仿真交接.md)。下列单刺、阵列和原始机理文档描述保留的旧求解器，不能作为新版理论依据。当前新研究依据是共同背板专用理论稿。
