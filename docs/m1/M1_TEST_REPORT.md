...
> **历史基线。** 本报告只覆盖 2026-07-27 的 `defined_geometry`、缓存和 CUDA
> 验收；不覆盖 2026-07-29 加入的材料特定生成器。相关材料测试与限制见
> [`../research/terrain/03_material_generation_implementation.md`](../research/terrain/03_material_generation_implementation.md)。
> 报告引用的本地 `results/` 产物现已清理，可由配置重新生成。
...
机器可读小基准和 smoke 地形库原位于
`results/m1_validation/benchmark.json` 与
`results/m1_validation/terrain_library/`（`results/` 已 gitignore），现已清理。

十工况 GPU 数据和机器可读报告原位于 `results/m1_gpu_suite/`，现已清理；提交版摘要见
