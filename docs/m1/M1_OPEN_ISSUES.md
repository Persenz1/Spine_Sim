...
1. 2026-07-27 的 `defined_geometry` 基线和 2026-07-29 的
   ...
   生成的主要剩余加速点之一。
2. geometry 已按完整针姿态检查球冠—锥段—圆柱杆的离散净空，但仍不是杆体分布
   接触本构。缺少球冠轴长、锥长、杆半径或露出长度时返回 unknown，不得翻译成无挂接。
3. 完整球仍只是局部有限针尖代理；forward-cap gate 已接入统一候选。真实过渡圆角、
   磨损、塑性和损伤演化仍未实现。
...
5. 旧正式设计曾要求砂纸、红砖、混凝土各 100 个 seed，共 300 个严格配对条件。
   当前正式 M1 catalog 为 0；旧 45 个 `defined_geometry` 条件已删除。必须先冻结
   若新版研究仍采用该 protocol，必须先冻结 family/subtype、seed 映射、10/5 μm
   同 realization 和完整哈希，再分批生成；本轮未运行大 campaign。
6. 文档引用的 `04_爪刺仿真器_失败问题总结与重启约束.md` 仍未在仓库提供。
   若后续补入，开始 canonical 大规模筛选前需做一次差异审查。
