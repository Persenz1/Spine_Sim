...
结果：**16/16 通过**，耗时 22.7 s。机器可读报告：
`results/m3_validation/dynamic_analytic_validation_v4.json`。
...
结果：**98 passed，另有 12 个 subtests 通过**，耗时 146.60 s；0 失败。该命令
包含工作区中用户正在开发的 M1 测试；本次提交仍不会纳入那些 M1 文件。
