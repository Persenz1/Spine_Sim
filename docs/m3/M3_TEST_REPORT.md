...
## 0. 2026-07-29 全量前续作结果

- 全仓最终回归：**110 passed，另有 15 subtests，0 failed**，耗时
  **91.70 s**；16 条 CuPy/NumPy 弃用警告不影响结果。
- 解析验收：**16/16**；本次显式传入正式 catalog 后，报告确认 300 条件及三类
  各 100 条，不再包含“catalog 不存在”的旧阻塞描述。
- 正式 P40/seed 41001 的真实 2×2 runner 到达 `path_end`，确定性换落点后最小
  净空 \(+91.45\) μm；resume 为 0.82 s，summary payload 和结果集哈希合并通过。
- Parquet 25.0.0 已安装，真实 summary 合并生成 zstd Parquet，字段类型抽查通过。
- 正式 summary 分片的事务型 SQLite 存储已通过 1,000 case 压测：
  144.8 case/s、约 570 B/case（合成小摘要），无逐 case 目录，1,000/1,000
  完成记录和 payload 哈希验证通过。
- 串行正式轨迹清单为每地形 130 条、全 catalog 39,000 条；已在 2,481.00 s
  内全部生成。39,000/39,000 数据文件、元数据、COMPLETE 标记和零残留锁清单
  检查通过；含 4 条额外验证轨迹的整个 track root 为 9.672 GiB。正式 worker
  已冻结为只读缓存。
- 三类 seed 41001 的全尺寸 5 μm 嵌套地形已生成并重算完整哈希，尺寸均为
  8041×29593，单条高度场 951,829,380 B，所有 10 μm 同节点逐值完全一致。
- `m3.5.0` 修复事件点重复采样后，P40 2×2、1 N、0.1 mm 的几何可行同落点
  对照在三个步长上均有 81 个统一位置稳态样本。0.5 ms 相对 0.25 ms 的
  中位/P10/Neff/最大针载差为 2.09%/21.93%/1.38%/0.58%；P240 的几何可行
  2 mm 对照仍为 5.92%/31.43%/11.19%/23.22%，时间步门未通过。
- 同 realization、同构型、同落点且两边杆净空均通过的 P40 10/5 μm、
  0.1 mm 对照也未通过：中位拉力、P10、Neff、最大针载差分别为
  9.01%、45.67%、5.70%、31.42%。
- 接触矩阵向量化、轨迹查询缓存和保留点在线杆净空终止保持可行路径摘要数值
  不变。P240 的 0.1 mm 最终基准为 2×2 2.33 s、4×4 8.15 s；6×6 七次失败
  落点由 202.89 s 降至 94.76 s。扣除“把固定沉降成本乘 1000”的旧外推偏差后，
  以 2×2、2 mm、0.5 ms 的 25.69 s 实测值理想化到 1 ms/100 mm，最乐观下界
  在 4 worker 下仍约 6.16 年；按设计平均针数线性估计约 23.31 年。因此
  full proxy launch 门保持关闭。
- 完整设计 manifest 为 1,344 个构型、1,209,600 个计划 case；另成功物化一个
  sandpaper/seed 41001/1 N、1,344-case 的正式 summary 分片，确认全部 case
  使用只读轨迹缓存且正式排名保持关闭。该分片未执行。
...
`results/m3_preflight_20260729/m3_5_dynamic_analytic_validation_online_clearance.json`。
...
当时结果：**107 passed，另有 15 个 subtests 通过**，耗时 138.56 s；0 失败。
本次续作后的最新全仓结果以第 0 节为准。
...
- 未将短程 smoke 用作排名；
- 未运行当时规划的完整有界收敛矩阵；现行 `m3.5.0` 计划为
  288 case/地形条件。
