...
from run_ijms_production import available_memory
from ijms_fine_campaign import inputs as fine_inputs, job_table as fine_jobs, worker, export_csv
...
DATA=PACKAGE/'data'/'fine'
STAGE='small'
PAGE=PROGRAM/'monitor'/'dist'/'fine_portable.html'
...
def hardware():
    ...
    # Coefficients are shared memory-mapped files; reserve common build/cache
    # space separately from each worker's Python and solver working arrays.
    shared_gib=2.0 if STAGE=='large' else .5
    per_worker_gib=.75
    workers=max(1,min(cpus,int((memory-(3+shared_gib)*2**30)//(per_worker_gib*2**30))))
...
def inputs(output,test_count=0):
    settings,base,designs=fine_inputs()
    if settings['model']!=MODEL_VERSION:raise ValueError('Model differs from frozen fine configuration')
    domains=[STAGE]
    for domain in domains:
        for sample in range(1 if test_count else 100 if domain=='small' else 20):
            for m in base['materials']:
                if test_count and m!=base['materials'][0]:continue
                stem=f"{m['material']}-{m['subtype']}-s{sample:03d}"
                for name in (stem+'.npy',stem+'.json',stem+'-r100-envelope20um-sigma10um.npy'):
                    if not (PACKAGE/'data/surfaces'/domain/name).is_file():raise FileNotFoundError('Missing packaged surface: '+domain+'/'+name)
    output.mkdir(parents=True,exist_ok=True)
    frozen=dict(settings=settings,base=base,designs=designs)
    config=output/'campaigns'/'scan.json';config.parent.mkdir(parents=True,exist_ok=True)
    if config.exists() and json.loads(config.read_text(encoding='utf-8'))!=frozen:raise ValueError('Output belongs to another configuration')
    write_json(config,frozen)
    return config

def ledger_counts(root=None):
...
def run(count,requested_workers,test=False):
    ...
    with lock_path.open('a+b') as lock:
        ...
        config=inputs(output,count if test else 0)
        settings,base,design_map=fine_inputs()
        ...
        total=sum(1 for _ in fine_jobs(STAGE,output))
        ...
        state=dict(status='RUNNING',stage=STAGE,mode='test' if test else 'production',process_id=os.getpid(),
        ...
        jobs=(j for j in fine_jobs(STAGE,output) if j['key'] not in done)
                ...
                while True:
                    ...
                    for future in ready:
                        ...
                        try:row=future.result()
                            ...
                            row=dict(design=job['design'],material=job['material'],preload_N=job['preload'],surface_set=job['domain'],role=job['role'],
...
def serve(port,test_default=False,open_browser=True):
    ...
    def snapshot():
        ...
        return dict(state=state,running=active,hardware=hardware(),counts=ledger_counts(),stage=STAGE,
        ...
        def do_POST(self):
            ...
            try:
                ...
                with start_lock:
                    ...
                    if count<0 or count>284300 or workers<0:raise ValueError('case数须在0至284300之间，worker数不能为负')
                    ...
                    command=[sys.executable,'-B',str(Path(__file__).resolve()),'--run','--count',str(count),'--workers',str(workers),'--stage',STAGE]
...
if __name__=='__main__':
    ...
    parser.add_argument('--stage',choices=['small','large'],default='small')
    parser.add_argument('--describe',action='store_true')
    args=parser.parse_args();STAGE=args.stage
    if args.count<0 or args.workers<0:parser.error('count and workers must be nonnegative')
    if args.run and args.test and not 1<=args.count<=32:parser.error('independent test count must be 1 to 32')
    DATA=DATA/STAGE
    DATA.mkdir(parents=True,exist_ok=True)
    if args.describe:
        print(json.dumps(dict(stage=STAGE,cases=sum(1 for _ in fine_jobs(STAGE,DATA)),hardware=hardware()),ensure_ascii=False));sys.exit(0)
